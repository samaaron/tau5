...
bool testValidationConflicts(TestContext& ctx) {
    ...
    // Test repl without dev mode
    {
        ...
        TEST_ASSERT(ctx, valid == false, "REPL in prod mode should fail validation");
        TEST_ASSERT(ctx, args.errorMessage.find("repl") != std::string::npos, 
            QString("Error should mention REPL, got: %1").arg(QString::fromStdString(args.errorMessage)));
    }
