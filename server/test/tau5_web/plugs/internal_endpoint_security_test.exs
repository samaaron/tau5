defmodule Tau5Web.Plugs.InternalEndpointSecurityTest do
  ...
  describe "CRITICAL: IP spoofing and localhost bypass attempts" do
    ...
    test "blocks localhost bypass attempts with various IP formats" do
      ...
      bypass_attempts = [
        {127, 0, 1, 1},           # Ubuntu localhost alias - should be blocked
        {127, 1, 1, 1},           # Still in 127.0.0.0/8 - should be blocked
        {127, 255, 255, 255},     # Broadcast - should be blocked
        {0, 0, 0, 0},             # All interfaces - should be blocked
        {0, 0, 0, 0, 0, 0, 0, 2}, # ::2 - should be blocked
        {10, 0, 0, 1},            # Private network - should be blocked
        {192, 168, 1, 1},         # Private network - should be blocked
      ]
    ...
    test "only allows true localhost IPs" do
      ...
      allowed_ips = [
        {127, 0, 0, 1},           # Standard IPv4 localhost
        {127, 0, 0, 2},           # Also valid in 127.0.0.x range
  ...
  describe "CRITICAL: Token timing attacks and validation" do
    ...
    test "token comparison is vulnerable to timing attacks" do
      ...
      tokens = ["bbbbbbbb", "abbbbbbb", "aabbbbbb", "aaabbbbb", 
                "aaaabbbb", "aaaaabbb", "aaaaaabb", "aaaaaaab"]
